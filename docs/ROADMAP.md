...
|------|------|
| 전체 진행률 | 0% |
| 현재 Phase | Phase 1 (MVP) 시작 전 |
| 다음 마일스톤 | Phase 1 Sprint 1 완료 - 게임 보드 렌더링 및 블록 드래그 |
| 시작일 | 2026-03-20 |
...
### Sprint 1: 게임 보드와 블록 시스템 (Week 1-2)
...
- ⬜ **블록 상수 정의** [복잡도: M]
  - `src/constants/blocks.ts`에 19종 블록 shape 배열 정의
  - 각 블록별 색상 매핑 (PRD 섹션 4.2 색상 팔레트 기반)
  - 블록 랜덤 선택 유틸 함수 (`src/utils/blockUtils.ts`)
  - 검증: 단위 테스트로 19종 블록이 올바른 shape를 가지는지 확인
...
- ⬜ **게임 상태 저장소 (Zustand)** [복잡도: M]
  ...
  - 액션: `placeBlock`, `removeLines`, `generateBlocks`, `resetGame`
...
- 보드 크기 계산에 `ResizeObserver` 활용하여 반응형 대응
- 셀 크기는 CSS 변수로 관리하여 일관성 유지
...
| 마일스톤 | 예상 완료일 | 산출물 |
|----------|------------|--------|
| M1: 프로토타입 | Sprint 1 종료 (2026-04-03) | 보드 렌더링 + 블록 드래그 데모 |
